...
class ManifestsCheckerTestCase(unittest.TestCase):
    ...
    @urlmatch(
        netloc=r"(.*\.)?raw\.githubusercontent\.com$", path=r".*this_is_from_a_pr.*$"
    )
    ...
    @urlmatch(
        netloc=r"(.*\.)?raw\.githubusercontent\.com$",
        path=r".*internalstaging.datastage.io.*$",
    )
    ...
    def setUp(self):
        ...
        self.patch2 = patch.object(
            ManifestsChecker, "_get_directories_from_repo", _get_directories_from_repo
        )
        self.patch1.start()
        self.patch2.start()
    ...
    def test_compare_manifests(self):
        with HTTMock(self.pr_manifest_mock, self.signed_off_manifest_mock):
            result = self.manifests_checker.compare_manifests(
                928,
                "<http://internalstaging.datastage.io|internalstaging.datastage.io>",
            )
    ...
    def test_whereis_version(self):
        with HTTMock(
            self.manifest_with_release_mock, self.manifest_without_release_mock
        ):
            ...
            result2 = self.manifests_checker.whereis_version(
                "revproxy", "1.17.6-ctds-1.0.1"
            )
