...
class ManifestsChecker:
    ...
    def whereis_version(self, looking_for, version):
        ...
        version_adoption = round(
            (num_of_envs_with_version / environments_count) * 100, 2
        )
        log.debug(
            "percentage of envs with [{}:{}]".format(looking_for, version_adoption)
        )
    ...
    def compare_manifests(self, pr_to_be_verified, signed_off_env):
        ...
        singed_off_manifest_url = "{}/{}/manifest.json".format(
            self.manifests_base_url, signed_off_env_host
        )
        log.info(
            "manifest with versions that have been tested: " + singed_off_manifest_url
        )
        signed_off_versions = self.httplib.fetch_json(singed_off_manifest_url)[
            "versions"
        ]

        diff = set(sorted(signed_off_versions.items())) - set(
            sorted(pr_manifest_versions.items())
        )
        diff_dict = dict(diff)
        if len(diff_dict.keys()) == 0:
            return ":white_check_mark: The `manifest.json` from <https://github.com/uc-cdis/cdis-manifest/pull/{0}/files|PR #{0}> and the one from <https://github.com/uc-cdis/cdis-manifest/blob/master/{1}/manifest.json|{1}> contain the same versions.".format(
                pr_to_be_verified, signed_off_env_host
            )
        else:
            return f"""
